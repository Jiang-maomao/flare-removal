...
def predict_flare_from_6_channel(input_tensor, gamma):
    ...
    merge_img_predicted_linear = adjust_gamma(
        deflare_img, gamma)+adjust_gamma(flare_img_predicted, gamma)
...
def predict_flare_from_6_channel_ours(input_tensor, gamma):
    # the input is a tensor in [B,C,H,W], the C here is 6
...
def predict_flare_from_3_channel(input_tensor, flare_mask, base_img):
    ...
    # flare_img_predicted = adjust_gamma_reverse(torch.clamp(
    #     merge_tensor_linear-input_tensor_linear, 1e-7, 1.0), gamma)

    masked_deflare_img = input_tensor*(1-flare_mask)+base_img*flare_mask
    # masked_flare_img_predicted = flare_img_predicted * \
    #     (1-flare_mask)+flare_img*flare_mask

    return masked_deflare_img
...
def get_highlight_mask(image, threshold=0.99, luminance_mode=False):
    ...
    if luminance_mode:
        # 3 channels in RGB
        luminance = 0.2126*image[:, 0, :, :] + 0.7152 * \
...
def blend_light_source(input_scene, pred_scene, threshold=0.99, luminance_mode=False):
    binary_mask = (get_highlight_mask(input_scene, threshold=threshold,
                   luminance_mode=luminance_mode) > 0.5).to("cpu", torch.bool)
    ...
    if kernel_size > 0:
        ...
        mask_rgb = torch.from_numpy(mask_rgb).to(
            input_scene.device, torch.float32)
